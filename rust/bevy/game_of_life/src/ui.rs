...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    commands
        ...
        .with_children(|parent| {
            parent.spawn_bundle(NodeBundle{  // bottom button BG border
                style: Style {
                ...
                .with_children(|parent| {
                    parent.spawn_bundle(NodeBundle {
                        ...
                        .with_children(|parent| {
                            parent.spawn_bundle(build_classic_button(&asset_server))
                                .with_children(|parent| {
...
fn build_classic_text(value: &str, asset_server: &Res<AssetServer>) -> TextBundle {
    TextBundle {
        ...
        ).with_alignment(TextAlignment{..Default::default()}),
        ..Default::default()
    }
}

fn button_system(
    mut interaction_query: Query<(&Interaction, &mut UiColor, &ClassicButton), (Changed<Interaction>, With<Button>)>,
    ...
    for (interaction, mut color, classic_button) in &mut interaction_query {
        match *interaction {
            Interaction::Clicked => {
